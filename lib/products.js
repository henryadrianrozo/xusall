...
//
// TODO: drop the App Store URL for XUsContacts into the empty string below the
// moment it ships. That's the only change needed to turn a card live: `href`
// becomes non-empty and `status` flips to 'live'.
export const PRODUCT_URLS = {
  democracy: 'https://democracy.xusall.com',
  notes: 'https://notes.xusall.com',
  pomodoro: 'https://pomodoro.xusall.com',
  calendar: 'https://calendar.xusall.com',
...
// Order matters: it drives the card grid, the footer nav and /llms.txt. Web
// products first, then the store apps, and anything not yet shipped last. The
// grid is two columns, so an odd number of products leaves the final card
// alone in its row; keeping the in-development one at the end means the gap
// sits beside a "coming soon" card rather than beside a live one.
export const PRODUCTS = [
  ...
  {
    key: 'calendar',
    // Indigo in the app (#2f55e0), which sits between the spectrum's blue and
    // violet, so the card splits the difference rather than picking one. It
    // also keeps this card distinct from XUsContacts, which is blue on blue.
    wordmark: [
      { text: 'X', color: 'var(--spectrum-blue)' },
      { text: 'Us', color: null },
      { text: 'Calendar', color: 'var(--spectrum-violet)' }
    ],
    status: 'live',
    description:
      'Turn a flyer, a screenshot or a message into a calendar event. An AI reads it, you check what it found, and one tap sends it to Google Calendar or downloads a file for Apple Calendar. No account, and nothing is saved.',
    platform: 'Web',
    href: PRODUCT_URLS.calendar,
    linkLabel: 'calendar.xusall.com'
  },
  {
    key: 'currency',
    wordmark: [
